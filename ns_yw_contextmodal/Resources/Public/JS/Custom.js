...
addEventListener("DOMContentLoaded", async (event) => {
    //const subTitleData = await getSubTitleData();
    ...
    if(getCookie("ymPageSlugLink") !== null || getCookie("ymPageSlugLink") !== '') {
        if (url.match(ymPagePattern)) {
            ...
            const ymPageSlugPart = url.split('/'+`${lang.slice(0,5)}` + '/');
            setCookie('ymPageSlugLink', ymPageSlugPart[1], 30);
            let redirectURL = `${window.location.protocol}//${window.location.hostname}/${lang.slice(0,5)}`;
    ...
    if(document.getElementsByClassName('ymPage').length > 0){
            ...
            if(getCookie("ymPageSlugLink") !== null || getCookie("ymPageSlugLink") !== '') {
                if (getCookie('ymPageSlugLink') === document.getElementsByClassName('ymPage')[i].getAttribute('id')) {
                    if(document.getElementsByClassName('ymPage')[i].getAttribute('menuitem') === '3'){
                        document.getElementsByClassName('ymPage')[i].addEventListener("click", (ev)=> {
                            let slug = ev.target.getAttribute('id');
                            let bsModalId = TYPO3.settings.TS.contextModaID;
                            let bsModal = document.querySelector(`#modal-${bsModalId}`);
                            //let appendId = convertToCamelCase(subTitleData[`${slug}`])
                            //        if(appendId) {
                            //    bsModal.addEventListener('show.bs.modal', function () {
                            //       history.pushState(null, null, appendId);
                            //    });
                            //    bsModal.addEventListener('hide.bs.modal', function () {
                            //        if (window.location.hash === appendId) {
		...
		const entryPoint = entryPointString.slice(0,5);
                    ...
                    if(colpos !== '3' && colpos !== '4'){
                        ...
                        fetch(getUrl, {
                            ...
                            modalBody.addEventListener('scroll', () => {
                                if (isScrolledToBottom(modalBody)) {
			...
			// load Swiper containers inside the modal (support both classes)
			modal.querySelectorAll('.contextSwiper, .ywContextSwiper').forEach(function (el) {
			    loadSwiper(el);
			});
...
function ym_useraccount_menu(event) {
	...
	if (event.target.id === 'yw_account_button') {
		...
		// Open payment modal from payment extension with ajax call instead
...
async function getSubTitleData()
{
    ...
    let ymPagePattern = /ym-(.+)/;
    let href = window.location.href;
    let entryPoint = href.split("/").pop();
    let slugArray = [];
    for(var i = 0; i < document.getElementsByClassName('ymPage').length; i++){
       let slug =  document.getElementsByClassName('ymPage')[i].getAttribute('id');
       slugArray.push(slug);
    }
    if (url.match(ymPagePattern)) {
        let lang = document.documentElement.lang;
        let ymPageSlugPart = url.split('/'+`${lang.slice(0,5)}` + '/');
        slugArray.push(ymPageSlugPart);
    }
    let arr = entryPoint.split('?');
    let finalURL = '';
    if (arr.length > 1 && arr[1] !== '') {
        finalURL = `${entryPoint}&type=1701869876&slug=${[slugArray]}`;
    } else {
        finalURL = `${entryPoint}?type=1701869876&slug=${[slugArray]}`;
    }
...
function loadAndAppendSubPageData(modal, modalBody, url)
{
	...
	// initialize any swipers that appeared with the newly appended content
	modal.querySelectorAll('.contextSwiper, .ywContextSwiper').forEach(function (el) {
	    loadSwiper(el);
	});
