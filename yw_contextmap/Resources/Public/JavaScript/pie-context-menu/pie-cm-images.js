(function (global, factory) {
	...
	// touchend & touchmove only recognized on menu div id=my_menu, since this is the active div where the touch takes place!
       //// document.getElementById("os-rez13").addEventListener("touchstart", (e) => touchstart(e));
        ...
        document.getElementById("os-rez13").addEventListener("touchend", (e) => touchend(e), false);
...
PieContextMenu.prototype.leftClickListener = function () {
    var that = this; // Ensure 'that' is properly scoped within the function

    // Select the YMviewer-canvas element
    var viewerCanvas = document.querySelector('.YMviewer-canvas');
    // Select the my_menu element
    var myMenu = document.getElementById('my_menu');

    // Function to handle the click event
    var handleClick = function (e) {
        var touchPoint = document.elementFromPoint(e.clientX, e.clientY);
        var button = e.which || e.button;
        if (button === 1) {
            if (!(touchPoint.classList.contains(that.menuablesClass))) {
                that.closeMenu();
            }
        }
    };

    // Attach the event listener if the YMviewer-canvas exists
    if (viewerCanvas) {
        viewerCanvas.addEventListener('click', handleClick);
    }

    // Attach the event listener if the my_menu exists
    if (myMenu) {
        myMenu.addEventListener('click', handleClick);
    }
};
...
//
    ...
    PieContextMenu.prototype.createMenuButton = function (button) {
        ...
        if (this.buttons[index].isNew) {
            ...
            button_icon = document.createElementNS(SVG_NS, "image");
        ...
        PieContextMenu.setMenuIcon(button_icon, iconUrl, radius, index, nob);
    };
    ...
    PieContextMenu.prototype.addButton = function (text, iconUrl) {
        this.buttons[this.numberOfButton] =
            new PieContextMenu.Button(this.numberOfButton, text, iconUrl);
        this.numberOfButton++;
        this.reset();
    };
    ...
    PieContextMenu.setMenuIcon = function (button_icon, iconUrl, radius, index, numberOfButton) {
        var iconSize = 40; // Set the icon size to 30px													   
        ...
        button_icon.setAttributeNS(null, 'href', iconUrl);
        button_icon.setAttribute("class", "pcm_icon");
        button_icon.setAttribute("x", dot.x - iconSize / 2);
        button_icon.setAttribute("y", -dot.y - iconSize / 2);
        button_icon.setAttribute("dy", "0.35em");
        button_icon.setAttribute("width", iconSize);
        button_icon.setAttribute("height", iconSize);
        return button_icon;
    };
