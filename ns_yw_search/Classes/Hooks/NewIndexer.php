...
class NewIndexer extends IndexerBase
{
        /**
    ...
    public function registerIndexerConfiguration(&$params, $pObj)
    {
        ...
        $customIndexer = array(
            '[CUSTOM] Specifed Pages Indexer',
            NewIndexer::KEY,
            'EXT:ns_yw_search/Resources/Public/Extension.gif'
        );
    ...
    {
        if ($indexerConfig['type'] == NewIndexer::KEY) {
            $table = 'pages';
    ...
    public function getPageRecords(array $uids, $whereClause = '', $table = 'pages', $fields = 'pages.*')
    {
        ...
        while ($row = $pageQuery->fetch()) {
            $row['fileObjects'] = $fileRepository->findByRelation('pages', 'twitter_image', $row['uid']);
            $pageRows[$row['uid']] = $row;
        }
        // \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($pageRows,__FILE__.''.__LINE__);die;
